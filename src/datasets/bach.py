...
class Bach(Dataset):
    ...
    def __init__(self, split, transform_lvl, datadir_base, folds_path, fold, patch_size, patch_extractor, n_samples=None, colorjitter=False, val_transform='identity', resize=None, min_resize=None, m=5,n=3):
        path = datadir_base or '/home/smounsav/scratch/datasets'
        folds_path = folds_path or '/home/smounsav/scratch/datasets/ICIAR2018_BACH_Challenge/Iciar_folds'
        self.name = 'bach'
        self.n_classes = 4
        self.split = split
        ...
        if n_samples is not None:
            ...
            with hu.random_seed(10):
                ...
                ind = np.hstack([np.random.choice(np.where(samples[:,1] == str(l))[0], n, replace=False)
                      for l in np.unique(samples[:,1])])
                # ind = np.random.choice(imgs.shape[0], n_samples, replace=False)
    ...
    def __getitem__(self, index):
        if self.split in ['test']:
            ...
            image_path, label = self.samples[torch.div(index, len(self.patch_extractor), rounding_mode='trunc')]
